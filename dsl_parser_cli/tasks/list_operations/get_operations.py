...
1. Get a virtualenv with cloudify-plugins-common installed - plugins may reference this
2. Get a plugin's source and put it relative to this source at folder X (see what X is below)
...
def op_hook(func, **kwargs):
    ...
    operations.append( { 'module' : op_module, 'name' : op_name, 'args' : func.func_code.co_varnames } )
...
def extract_module_paths( plugin_name ):
    ...
    module_paths = []
    p = Popen(['pip', 'show', '-f', plugin_name ], stdin=PIPE, stdout=PIPE, stderr=PIPE)
